"""Formate-route SCP flowsheet builder.

Public function
---------------
build_formate_system(params, economics) -> tuple[bst.System, dict[str, float]]
...
sterilization → fermentation → recovery → product storage → WWT → recycle:
  ...
  formate_feed   → ST101 (StorageTank) ─┐
...
Note on Yxs: the formate route has Yxs = 0.06 g biomass / g formate
(Claassens et al. 2020), ~15× lower than fructose.  This drives a large S0
and Q to achieve the same production target — expect a substantially larger
reactor volume and higher substrate throughput than the sugar/acetate routes.
...
--------------------------
formate_feed   : FormicAcid at S0 g/L, Q m³/h      → price = feedstock_price
...
Units
-----
ST101  — bst.StorageTank        : formate feedstock storage (7 days — Framework §9.2)
...
ST201  — bst.StorageTank        : seed formate storage (7 days — Framework §9.2)
...
SR103  — SeedBioreactor         : Stage-3 seed fermenter (directly inoculates R101), single sterilized inlet
...
def build_formate_system(
    params: RouteParams,
    economics: EconomicBasis,
) -> tuple[bst.System, dict[str, float]]:
    """Build and return the formate-route BioSTEAM system.
    ...
    ----------
    params : RouteParams
        Per-route kinetics and design parameters from common.parameters.get('formate').
    economics : EconomicBasis
    ...
    S0     = d['S0']      # g/L  — formate (formic acid) feed concentration
    ...
    molar = compute_O2_CO2_H2O_coefficients(
        substrate_formula='CH2O2',      # Formic acid — Framework §7
        biomass_formula=biomass_formula,
        Yxs=params.Yxs,
    )
    MW_FormicAcid = chems['FormicAcid'].MW
    MW_NH3        = chems['NH3'].MW
    nh3_wt = molar['NH3'] * MW_NH3 / MW_FormicAcid   # g NH3 / g FormicAcid (mass basis)

    growth_rxn = build_growth_reaction(
        substrate_id='FormicAcid',
        substrate_formula='CH2O2',
        Yxs=params.Yxs,
        epsilon=params.epsilon,
    ...
    #           = molar['H2O'] mol H2O / mol FormicAcid × (MW_H2O / MW_FormicAcid)
    #             × formate_kgh × epsilon
    ...
    h2o_mass_coeff = molar['H2O'] * MW_H2O / MW_FormicAcid           # kg H2O / kg FormicAcid consumed
    W_rxn          = h2o_mass_coeff * formate_kgh * params.epsilon    # kg/h H2O from reaction

    biomass_after_centrifuge = (formate_kgh * params.epsilon
    ...
    # 8. Feed streams
    # ------------------------------------------------------------------
    formate_feed = bst.Stream(
        'formate_feed',
        FormicAcid=formate_kgh,
        units='kg/hr',
    ...
    # ------------------------------------------------------------------
    formate_storage   = bst.StorageTank('ST101', ins=formate_feed,   tau=STORAGE_TANK_TAU)
    ...
    feed_mixer = bst.MixTank(
        'M101',
        ins=[formate_storage-0, nutrients_storage-0, nh3_storage-0,
    ...
    system = bst.System(
        'formate_system',
        path=[formate_storage, nutrients_storage, nh3_storage,
