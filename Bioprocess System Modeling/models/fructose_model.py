"""Fructose-route SCP flowsheet builder.

Public function
---------------
build_fructose_system(params, economics) -> tuple[bst.System, dict[str, float]]
...
sterilization → fermentation → recovery → product storage → WWT → recycle:
  ...
  fructose_feed  → ST101 (StorageTank) ─┐
...
recycle returns a small additional flow (~6 % of process water demand for the
fructose route) that slightly overshoots the design volume, which is acceptable
for a first-pass TEA.  The 75 % recycle fraction (WWT_WATER_RECYCLE_FRACTION)
keeps the loop a contraction mapping (gain = 0.75 < 1 → converges).
...
--------------------------
fructose_feed  : Fructose at S0 g/L, Q m³/h       → price = feedstock_price
...
Units
-----
ST101  — bst.StorageTank        : fructose feedstock storage (7 days — Framework §9.2)
...
ST201  — bst.StorageTank        : seed fructose storage (7 days — Framework §9.2)
...
def build_fructose_system(
    params: RouteParams,
    economics: EconomicBasis,
) -> tuple[bst.System, dict[str, float]]:
    """Build and return the fructose-route BioSTEAM system.
    ...
    ----------
    params : RouteParams
        Per-route kinetics and design parameters from common.parameters.get('fructose').
    economics : EconomicBasis
    ...
    S0     = d['S0']      # g/L  — fructose feed concentration
    ...
    molar = compute_O2_CO2_H2O_coefficients(
        substrate_formula='C6H12O6',    # Fructose — Framework §7
        biomass_formula=biomass_formula,
        Yxs=params.Yxs,
    )
    MW_Fructose = chems['Fructose'].MW
    MW_NH3      = chems['NH3'].MW
    nh3_wt = molar['NH3'] * MW_NH3 / MW_Fructose   # g NH3 / g Fructose (mass basis)
    ...
    #           = molar['H2O'] mol H2O / mol fructose × (MW_H2O / MW_Fructose)
    #             × fructose_kgh × epsilon
    ...
    h2o_mass_coeff = molar['H2O'] * MW_H2O / MW_Fructose         # kg H2O / kg fructose consumed
    W_rxn          = h2o_mass_coeff * fructose_kgh * params.epsilon  # kg/h H2O from reaction

    biomass_after_centrifuge = (fructose_kgh * params.epsilon
    ...
    # 8. Feed streams
    # ------------------------------------------------------------------
    fructose_feed = bst.Stream(
        'fructose_feed',
        Fructose=fructose_kgh,
        units='kg/hr',
    ...
    # ------------------------------------------------------------------
    fructose_storage  = bst.StorageTank('ST101', ins=fructose_feed,  tau=STORAGE_TANK_TAU)
    ...
    feed_mixer = bst.MixTank(
        'M101',
        ins=[fructose_storage-0, nutrients_storage-0, nh3_storage-0,
    ...
    #   centrifuge-1 : liquid effluent (residual fructose, water, trace biomass)
    ...
    system = bst.System(
        'fructose_system',
        path=[fructose_storage, nutrients_storage, nh3_storage,
