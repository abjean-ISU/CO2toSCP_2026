...
%READ ME
%This program is intended to predict kLa, final dissolved gas
%concentration, droplet residence time, and liquid holdup based on provided
%system parameters (tower height, starting dissolved gas concentration,
%mean droplet size/droplet size distribution [histogram], tower operating 
%pressure, liquid flowrate, liquid temperature, and gas composition). This 
%program is intended only for use for single-component quiscent gas systems. 
...
towerHeight = 3; %m
towerDiameter = 0.25; %m 

%Input manifold parameters
d_orifice = 0.8/1000; %droplet manifold orifice diameter (m)
N_o = 200; %number of orifices in droplet manifold 

%Input operating conditions 
Q = 4.5/1000/60; %liquid flowrate (m3/s)
T = 18.42 + 273.15; %liquid temperature (K)
P = 2.4; %operating (headspace) pressure (atm) 

%Input relavelt gas composition information 
DAB = 1.67E-9; %diffusion coefficient of gas in water at correct Temp (m2/s)
H = (1/(exp(-159.854+8741.68/T+21.6694*log(T)-1.10261E-3*T)))/55324; %Henry's Constant for CO2 (atm-m3/mol)
c0 = 0; %initial dissolved gas concentration (g/L)
